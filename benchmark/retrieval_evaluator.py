...
DEFAULT_OUT_DIR = "/home/vinhpq/mem_baseline/mem0test/eval_out"
...
def eval_dataset(
    ...
    category_data: Dict[str, Dict] = {}  # category -> {macro_sums, micro_sums, counts, per_record}
    ...
    for idx in iterator:
        ...
        if category not in category_data:
            ...
            if use_all:
                category_data[category]["macro_sums"][ALL_K_SENTINEL] = {m: 0.0 for m in METRIC_KEYS}
                category_data[category]["counts"][ALL_K_SENTINEL] = 0
                category_data[category]["per_record"][ALL_K_SENTINEL] = []
                category_data[category]["micro_sums"][ALL_K_SENTINEL] = {"tp_evidence": 0.0, "retrieved": 0.0, "gold": 0.0,
                                                                          "rel_chunks": 0.0, "dcg": 0.0, "idcg": 0.0}

        cat_data = category_data[category]
        for k, m in per_k_metrics.items():
            cat_data["per_record"].setdefault(k, []).append((idx, qid, m))
            for mk in METRIC_KEYS:
                cat_data["macro_sums"][k][mk] += m.get(mk, 0.0)
            cat_data["counts"][k] = cat_data["counts"].get(k, 0) + 1

        for k, c in per_k_counts.items():
            ms = cat_data["micro_sums"].setdefault(k, {"tp_evidence": 0.0, "retrieved": 0.0, "gold": 0.0,
                                                        "rel_chunks": 0.0, "dcg": 0.0, "idcg": 0.0})
            for key in ms:
                ms[key] += c[key]
    ...
    for cat, cat_data in category_data.items():
        cat_macro_avgs = {
            k: {m: (cat_data["macro_sums"][k][m] / cat_data["counts"][k]) if cat_data["counts"][k] > 0 else 0.0 
                for m in METRIC_KEYS}
            for k in cat_data["counts"].keys()
        }
        cat_micro_avgs = {k: _counts_to_metrics(c, precision_mode=precision_mode) 
                         for k, c in cat_data["micro_sums"].items()}
        category_avgs[cat] = {
            "macro_avgs": cat_macro_avgs,
            "micro_avgs": cat_micro_avgs,
            "counts": cat_data["counts"]
        }

    # trả thêm danh sách skip và category averages
    return macro_avgs, micro_avgs, counts, failed, per_record, micro_sums, skipped_no_evidence, category_avgs
...
def print_report_both(macro_avgs, micro_avgs, counts, failed, skipped_no_evidence, category_avgs=None):
    ...
    if category_avgs:
        ...
        for cat in sorted(category_avgs.keys()):
            ...
            print(f"\n--- Category: {cat} ---")
            for k in sorted(cat_data["counts"].keys()):
                ma = cat_data["macro_avgs"][k]
                mi = cat_data["micro_avgs"][k]
                label = _k_label(k)
                print(f"  @ {label} (n={cat_data['counts'][k]})")
                print(f"    Macro  | P: {ma['precision']:.4f}  R: {ma['recall']:.4f}  F1: {ma['f1']:.4f}  nDCG: {ma['ndcg']:.4f}")
                print(f"    Micro  | P: {mi['precision']:.4f}  R: {mi['recall']:.4f}  F1: {mi['f1']:.4f}  nDCG: {mi['ndcg']:.4f}")
...
def save_eval_results(
    ...
    category_avgs: Dict[str, Dict] = None,
) -> str:
    """
    ...
    payload: Dict[str, Any] = {
        ...
        "category_avgs": {
            cat: {
                "macro_avgs": _dict_with_str_k(cat_data["macro_avgs"]),
                "micro_avgs": _dict_with_str_k(cat_data["micro_avgs"]),
                "counts": _dict_with_str_k(cat_data["counts"])
            }
            for cat, cat_data in category_avgs.items()
        } if category_avgs else {},
    }
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
...
def main():
    ...
    for pth in args.input:
        ...
        macro_avgs, micro_avgs, counts, failed, per_record, micro_sums, skipped_no_evidence, category_avgs = eval_dataset(
            dataset=dataset,
            ks=ks,
            use_all=use_all,
        ...
        print(f"\n=== File: {pth} ===")
        print_report_both(macro_avgs, micro_avgs, counts, failed, skipped_no_evidence, category_avgs)
...
python retrieval_evaluator.py \
  --input /home/vinhpq/mem_baseline/mem0/locomo_retrieve_result.json \
  --out-dir /home/vinhpq/mem_baseline/mem0/eval_out

python3 retrieval_evaluator.py \
  --input /mnt/hungpv/projects/memory/amem/locomo_results_20260102_161503/retrieval_results_20260102_161503.json \
  --out-dir /home/vinhpq/mem_baseline/amem_temp
"""