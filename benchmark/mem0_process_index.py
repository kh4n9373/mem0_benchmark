...
# Add project root to sys.path to allow importing mem0 package
    ...
    for conv_idx, conversation in enumerate(tqdm(dataset, desc=f"Indexing (shard {shard_id}/{num_shards})")):
        ...
        for session in dialogs:
            ...
            for message in session['messages']:
                ...
                # Skip Assistant messages (Melanie) to avoid polluting index with chet-chat/questions
                if role == "Melanie" or role == "Assistant":
                    continue
        ...
        try:
            ...
            for i, chunk_data in enumerate(tqdm(chunks, desc=f"Adding chunks [{conv_id}]", leave=False)):
                ...
                except (openai.APIConnectionError, openai.InternalServerError, openai.APITimeoutError, openai.RateLimitError, RuntimeError, Exception) as e:
                    # TRANSIENT ERROR: Do NOT mark as completed. Will retry next run.
                    # Includes OOM (RuntimeError) and other unexpected system glitches.
                    print(f"\n  ⚠️  Transient/System Error processing chunk {i} for {conv_id}: {e}")
...
def main():
    ...
    # Graph Args
    parser.add_argument("--graph_provider", default=None, help="Graph provider (e.g. neo4j)")
    parser.add_argument("--graph_url", default=None, help="Graph database URL")
    parser.add_argument("--graph_username", default=None, help="Graph database username")
    parser.add_argument("--graph_password", default=None, help="Graph database password")
